...
public class Car { // 클래스 이름은 대문자로 시작함
    ...
    private String productName; // 인스턴트 필드
    String color; // 인스턴트 필드
    int maxSpeed; // 인스턴트 필드
    ...
    // 생성자의 목적은 클래스가 객체로 만들어 질 때 해당 인스턴스 필드를 초기화 하는 것이 주 목적임
    Car() {
        productName = "이름없은 차";
        color = "색상도 없음";
        maxSpeed = 0;
    ...
    Car(String name, String color, int speed) {
        productName = name;
        this.color = color; // 매개변수로 전달된 변수와 인스턴트 필드의 이름이 같은 경우 this 필요
        maxSpeed = speed;
    }
    // 세터 메소드라 부른다.
